...
Examples:
  hpcloud servers:add my_server 7 small -k key1          # Create a new server named 'my_server' using an image, flavor and a key:
  hpcloud servers:add winserv 100006567 large -k winpair -s allowsRDP -p ./winpair.pem # Create a windows server with the specified key, security group, and private key to decrypt the password:
  hpcloud servers:add my_server 7 large -k key1 -s sg1   # Create a new server named 'my_server' using an image, flavor, key and security group:
  hpcloud servers:add my_server 7 small -k key1 -m this=that     # Create a new server named 'my_server' using an image, flavor, key and metadata this=that:
  hpcloud servers:add my_server 7 xlarge -k key1 -z az-2.region-a.geo-1  # Create a new server named 'my_server' using a key for  availability zone `az-2.region-a.geo-1`:
      DESC
