...
describe "dns:records:update command" do
  ...
  context "when creating dns with name description" do
    it "should show success message" do
      ...
      rsp = cptr("dns:records:update #{@dns1.name} www.clitest1.com. A 10.0.0.2")

      rsp.stderr.should eq("")
      @new_dns_id = rsp.stdout.scan(/'([^']+)/)[2][0]
      rsp.stdout.should eq("Updated DNS record 'www.clitest1.com.' with id '#{@new_dns_id}'.\n")
      rsp.exit_status.should be_exit(:success)
      rsp = cptr("dns:records -c name,type,data -d X #{@dns1.name}")
      rsp.stdout.should match("www.clitest1.com.XAX10.0.0.2\n")
    end
