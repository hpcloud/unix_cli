...
#      response = capture(:stdout){ HP::Cloud::CLI.start(['acl:set', ':acl_container', 'authenticated-read']) }
...
#    it "should report success" do
#      response = capture(:stdout){ HP::Cloud::CLI.start(['acl:set', ':acl_container/foo.txt', 'authenticated-read']) }
