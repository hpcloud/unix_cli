...
describe "buckets:remove command" do
  ...
  context "when user doesn't have permissions to remove" do
    ...
    before(:all) do
      @other_user.put_container('other_users_bucket')
    ...
    it "should show error message" do
      ...
      @response.should eql("You don't have a bucket named 'other_users_bucket'.\n")
    end
  ...
  context "when user owns bucket and it exists" do
    ...
    before(:all) do
      @kvs.put_container('bucket_to_remove')
    ...
    it "should remove bucket" do
      lambda{ @kvs.get_container('bucket_to_remove') }.should raise_error(Fog::HP::Storage::NotFound)
    end
